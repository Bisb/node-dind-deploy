...
app.post('/deploy', async (req, res) => {
  ...
  try {
    ...
    if (config.stackPath) {
      ...
      const commands = config.commands && Array.isArray(config.commands) && config.commands.length > 0
        ? config.commands
        : [`docker compose ${fileArgs} pull`, `docker compose ${fileArgs} up -d --remove-orphans`];

      for (const cmd of commands) {
        const result = await executeCommand(cmd);
        results.push({step: cmd, ...result});
      }
    } else {
